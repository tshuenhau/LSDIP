...
exports.sendEmail = functions.firestore
    ...
    .onCreate(async (snap, context) => {
        if (snap.data().customerEmail.length > 0) {
            ...
            const orderItems = await Promise.all(order.orderItemIds.map(async (orderItemId) => {
                const orderItemDoc = await db.collection('orderItem').doc(orderItemId).get();
            ...
            const orderItemRows = orderItems.map((orderItem) => {
                return `
                    <tr>
                        <td>${orderItem}</td>
                    </tr>
            ...
            const pickupDetails = order.pickup ? `Selected Laundry Pickup: $${order.pickupCost.toFixed(2)}` :  'No Laundry Pickup';

            const mailOptions = {
                from: `lagoonlaundryplq@gmail.com`,
                to: order.customerEmail,
                subject: `Lagoon Laundry - Order Confirmation (#${order.invoiceNumber})`,
                html: `
                    <html>
                        <head>
